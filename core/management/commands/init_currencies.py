...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            for currency_data in self.CURRENCIES:
                currency = Currency.objects.get(currency_code=currency_data["currency_code"])
            ...
            self.stdout.write(self.style.WARNING(f"  ⚠️  Уже существует: {source.name}"))
        ...
        self.stdout.write(f"  • Источников курсов: {CurrencyRateSource.objects.count()}")
        ...
        self.stdout.write(f"  • Отслеживаемых валют: {source.tracked_currencies.count()}")

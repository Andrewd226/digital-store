...
class BaseService(ABC, Generic[TDTO, TResult]):
    ...
    def start_sync(self, triggered_by: str = "celery") -> SupplierCatalogSync:
        self.sync_record = SupplierCatalogSyncDAO.create_running(
            supplier=self.supplier, triggered_by=triggered_by,
        )
        logger.info("Начало синхронизации для поставщика %s (ID=%s)", self.supplier.name, self.supplier.id)
    ...
    def complete_sync(self, status: str | None = None, error_log: str = "") -> SupplierCatalogSync:
        ...
        sync_record = SupplierCatalogSyncDAO.complete(
            sync_record=self.sync_record, status=status,
            total_items=self.stats.total, created_items=self.stats.created,
            updated_items=self.stats.updated, skipped_items=self.stats.skipped,
            failed_items=self.stats.failed, error_log=error_log,
        )

        logger.info("Завершение синхронизации для %s: статус=%s, всего=%s", self.supplier.name, status, self.stats.total)
        return sync_record
    ...
    def _update_stats(self, result: TResult) -> None:
        if hasattr(result, "created") and result.created: self.stats.created += 1
        if hasattr(result, "updated") and result.updated: self.stats.updated += 1
        if hasattr(result, "skipped") and result.skipped: self.stats.skipped += 1
        if hasattr(result, "failed") and result.failed: self.stats.failed += 1
